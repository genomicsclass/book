...
```
## Error in pd$Status: $$ operator not defined for this S4 class
```
...
```
## Error in eval(expr, envir, enclos): object 'normalIndex' not found
```
...
```
## Error in density(meth[, i], from = 0, to = 1): object 'i' not found
```
...
```
## Error in eval(expr, envir, enclos): object 'cancerlIndex' not found
```
...
```
## Error in library(rafalib): there is no package called 'rafalib'
```
...
```
## Error in eval(expr, envir, enclos): could not find function "splot"
```
