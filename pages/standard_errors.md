...
As a first step we need to define the *variance-covariance matrix*. For a vector of random variable, $$\mathbf{Y}$$, we define $$\boldsymbol{\Sigma}$$ as the matrix with the $$i,j$$ entry:

$$ \Sigma_{i,j} \equiv \mbox{Cov}(Y_i, Y_j) $$

The covariance is equal to the variance if $$i = j$$ and equal to 0 if the variables are independent. In the kinds of vectors considered up to now, for example, a vector $$\mathbf{Y}$$ of individual observations $$Y_i$$ sampled from a population, we have assumed independence of each observation and assumed the $$Y_i$$ all have the same variance $$\sigma^2$$, so the variance-covariance matrix has been:
...
Later, we will see a case, specifically the $$\hat{\boldsymbol{\beta}}$$, that has non-zero entries in the off diagonal elements of $$\boldsymbol{\Sigma}$$.
